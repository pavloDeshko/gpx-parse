...
var _ParseV10 = function(gpx) {
	...
	metadata = new GpxMetaData(gpx.$.creator, gpx.time, extent);// wrong!!
...
exports.buildGpx = function(result, callback){
  var xmlJs = {
    gpx:{
      $: {version:"1.1", creator:"gpx-parse"},
      _: " ", // self closing are valid or not?
      metadata: _writeMeta(result.metadata),
      wpt: _writeWaypoints(result.waypoints),
      rte: _writeRoutes(result.routes),
      trk: _writeTracks(result.tracks)
    }
  };
  
  var xml = (new xml2js.Builder()).buildObject(xmlJs);
  callback(null, xml); 
};

//each function returns array of wrapped xml entities
var _writeMeta = function(data){//check types!
  return [{
    name: ,
    desc: ,
    author: ,
...
var _writeWaypoints = function(points){
  return points.map(point=>_wrapWP(point));
};

var _writeRoutes = function(routes){
  return routes.map(route=>{
      name: route.name ? [route.name] : [],
      desc: route.desc ? [route.desc] : [],
      cmt: route.cmt ? [route.cmt] : [],
      rtept: _writeWaypoints(route.points),
  });
};

var _writeTracks = function(tracks){
  return tracks.map(track=>{
    name : track.name ? [track.name] : [],
    trkseg : track.segments.map(segment=>{trkpt: _writeWaypoints(segment)}) //add extensions
  });
};

function _wrapWP(waypoint){
  var wrapped = {
      $: {
  ...
  if(waypoint.elevation) wrapped[ele] = [waypoint.elevation.toString()];
  
  var otherAttrs = [time, name, cmt, desc, src, type];
  for(var i = 0; i<otherAttrs.length; i++){
    if (waypoint[otherAttrs[i]]){
      wrapped[otherAttrs[i]] = [waypoint[otherAttrs[i]]];
    }
  }
